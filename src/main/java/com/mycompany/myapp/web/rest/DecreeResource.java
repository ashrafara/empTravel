...
public class DecreeResource {
    ...
    @GetMapping(value = "/public/decrees/count-country-day-emp/xlsx", produces = "application/vnd.ms-excel")
    public ResponseEntity<byte[]> countCountrydayemployeeAsXSLX() {
        List<Object[]> data = decreeRepository.findAllCountCountrydayemployee();
        String[] columns = { "dayCount", "decreeCount", "employee name" };
        Workbook workbook = new XSSFWorkbook();
        Sheet sheet = workbook.createSheet("Companies");
        Font headerFont = workbook.createFont();
        headerFont.setBold(true);
